...
async function cargarAlmacenes() {
  try {
    ...
    if (payload && Array.isArray(payload.items))
      almacenes = payload.items.map(mapServerAlmacen);
    else if (Array.isArray(payload)) almacenes = payload.map(mapServerAlmacen);
    else almacenes = [];
    renderizarAlmacenes();
...
function mostrarModalEditar(id) {
  const almacen = almacenes.find((a) => String(a.id) === String(id));
  if (!almacen) return;
...
async function crearAlmacen() {
  ...
  try {
    ...
    const res = await api.post('/almacenes', payload);
    const created = res && res.data ? res.data : res;
    almacenes.push(mapServerAlmacen(created));
    renderizarAlmacenes();
    actualizarEstadisticas();
    actualizarPanelesInferiores();
    ocultarModal();
    ...
    mostrarToast('Error al crear almacén', 'error');
  }
}
...
async function editarAlmacen() {
  ...
  try {
    ...
    const res = await api.put(`/almacenes/${id}`, payload);
    const updated = res && res.data ? res.data : res;
    const mapped = mapServerAlmacen(updated);
    almacenes = almacenes.map((a) =>
      String(a.id) === String(id) ? mapped : a,
    );
...
async function eliminarAlmacen(id) {
  ...
  try {
    ...
    almacenes = almacenes.filter((a) => String(a.id) !== String(id));
    renderizarAlmacenes();
    actualizarEstadisticas();
    actualizarPanelesInferiores();
...
function guardarAlmacenes() {
  ...
  console.log('Almacenes (local) actualizados:', almacenes.length, 'elementos');
}
