...
function actualizarEstadisticasEstado() {
  ...
  const estados = {
    ...
    // NUEVO: Embarques en retraso
    delayed: {
      nombre: "En retraso",
      color: "red",
      cantidad: embarques.filter((e) => estaEnRetraso(e)).length,
    },
  };
...
// Función para detectar embarques en retraso
function estaEnRetraso(embarque) {
  // Convertir fechaEstimada "dd/mm/yyyy - HH:mm" a Date
  const [fechaPart, horaPart] = embarque.fechaEstimada.split(" - ");
  const [dia, mes, anio] = fechaPart.split("/");

  // Crear fecha estimada (formato: yyyy-mm-dd)
  const fechaEstimada = new Date(`${anio}-${mes}-${dia}T${horaPart}`);

  // Considerar en retraso si pasó la fecha estimada y NO está completado
  return fechaEstimada < new Date() && embarque.estado !== "completed";
}
