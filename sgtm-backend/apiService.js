/**
 * Servicio API para integración con el frontend
 * Este archivo debe ser copiado al frontend y adaptado según sea necesario
 */
...
class ApiService {
  ...
  // Tareas
  async getTareas(page = 1, limit = 10, filters = {}) {
  ...
  // Embarques
  async getEmbarques(page = 1, limit = 10, filters = {}) {
    const params = new URLSearchParams({ page, limit, ...filters });
  ...
  // Rutas
  async getRutas(page = 1, limit = 10, filters = {}) {
    const params = new URLSearchParams({ page, limit, ...filters });
  ...
  // Facturas
  async getFacturas(page = 1, limit = 10, filters = {}) {
    const params = new URLSearchParams({ page, limit, ...filters });
  ...
  // Personal
  async getPersonal(page = 1, limit = 10, filters = {}) {
    const params = new URLSearchParams({ page, limit, ...filters });
  ...
  // Embarcaciones
  async getEmbarcaciones(page = 1, limit = 10, filters = {}) {
    const params = new URLSearchParams({ page, limit, ...filters });
  ...
  // Almacenes
  async getAlmacenes(page = 1, limit = 10, filters = {}) {
    const params = new URLSearchParams({ page, limit, ...filters });
  ...
  async getHistorialExportaciones(limit = 10) {
    return this.request(`/exportar/historial?limit=${limit}`);
  }

  async descargarExportacion(id) {
    const response = await fetch(`${API_BASE}/exportar/descargar/${id}`, {
      headers: {
    ...
    a.href = url;
    a.download = `exportacion_${id}.${blob.type.split('/')[1]}`;
